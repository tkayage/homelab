...
Use SOPS v3.13.0 with age. Keep the age private key in `/home/tonny/.config/homelab/age/keys.txt` (0600), create its cluster copy imperatively during bootstrap, and commit only its public recipient and SOPS ciphertext. Run decryption in a config-management-plugin sidecar. The plugin copies the application source to a temporary directory, decrypts `*.enc.yaml` files to their corresponding `*.yaml` paths, and runs Kustomize there, so plaintext never enters git or the Argo repository working tree.
